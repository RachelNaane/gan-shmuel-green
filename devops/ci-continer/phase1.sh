...
mkdir test 
cd test || { echo "'cd test' failed "; curl localhost/send_mail; exit 1;}
git clone https://github.com/RachelNaane/gan-shmuel-green.git || { echo "clone from rep failed"; curl localhost/send_mail; exit 1;}
wait

# up env-testing
cd $root_weight || { echo "'cd $(root_weight)' the current path is $(pwd)"; curl localhost/send_mail; exit 1;}

echo -e "APP_PORT=8086\nDB_PORT=8087\nHOST_VOLUME=$weight_host_volume\nMYSQL_VOLUME=$weight_mysql_volume\nNETWORK=test-net" > .env
docker-compose build --no-cache || { echo "could not build the image for weight"; curl localhost/send_mail; exit 1; }
wait
docker-compose up -d || { echo "could not run the dockers for weight "; curl localhost/send_mail; exit 1;}
wait


cd $root_billing || { echo "'cd $(root_billing)' the current path is $(pwd)" |tee $full_score_path_weight $full_score_path_billing ; curl localhost/send_mail; exit 1;}
...
# run test
bash $root_weight/tests/test.sh || { echo "no test.sh file found for weight";  curl localhost/send_mail; exit 1;}
exitcode_weight=$?
bash $root_billing/tests/test.sh || { echo "no test.sh file found for billing"; curl localhost/send_mail; exit 1;}
exitcode_billing=$?
...
cd $root_billing || { echo "'cd $(root_billing)' the current path is $(pwd)"; curl localhost/send_mail; exit 1;}
docker-compose down -v
wait

# delete test dir
cd /app || { echo "'cd /app' failed"; curl localhost/send_mail; exit 1;}
rm -fr test
...
echo -e "\n\n$message \n$billing_result \n$weight_result \n$conclusion" >> $full_score_path_weight || { echo "problem"; curl localhost/send_mail; exit 1;}
echo -e "\n\n$message \n$billing_result \n$weight_result \n$conclusion" >> $full_score_path_billing || { echo "problem"; curl localhost/send_mail; exit 1;}
